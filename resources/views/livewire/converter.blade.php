            ...
            <div class="container">
                ...
                <div class="text">
                    ...
                    <div class="call-to-action">
                        <input id="files" type="file" wire:model="files" multiple wire:change="inputChanged">
                        <label for="files">Wgraj swoje pliki</label>
                        <p>maksymalny rozmiar pliku to 5 MB</p>
                        
                        @if ($errors->any())
                            <div class="upload-error">Oczekiwanymi plikami są obrazki, zdjęcia do 5 MB każdy</div>
                        @endif

                        @if ($showFiletypePicker && count($errors) === 0)
                            <div class="choose-target-filetype">
                                Konwertuj pliki na:
                                <select wire:model="targetFiletype">
                                    <option value="png" selected>PNG</option>
                                    <option value="jpg">JPG</option>
                                    <option value="gif">GIF</option>
                                    <option value="tiff">TIFF</option>
                                </select>
                                <button type="submit">Konwertuj pliki</button>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        @if($filesSaved)
            <section class="converter">
                <div class="converter__top">
                    ...
                    <div class="progress">
                        Przekonwertowano <b>{{ count($files) }} z {{ count($files) }}</b>
                    </div>
                </div>
                            ...
                            <div class="file">
                                <div class="file__summary">
                                    <div class="file__preview file__preview--success">
                                        <img src="{{ asset('storage/' . $filesDir . '/' . $file->convertedFilename) }}" alt="">
                                    </div>
                                    <div class="file__details">
                                        <div class="file__name">{{ $file->convertedFilename }}</div>
                                        <div class="file__size">{{ (Storage::size('public/' . $filesDir . '/' . $file->convertedFilename))/1000000 }} MB</div>
                                        <div class="file__added-at">Zmodyfikowano o {{ date('d.m.Y H:i', Storage::lastModified('public/' . $filesDir . '/' . $file->convertedFilename)) }}</div>
                                    </div>
                                </div>
                                <div class="file__action" data-tippy-content="Pobierz plik" wire:click="download({{ $i }})">
                ...
                <div class="converter__actions">
                    ...
                    {{-- <button class="download-all" data-tippy-content="Pobierz wszystkie pliki w formacie .zip" wire:click="downloadZip">
                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
